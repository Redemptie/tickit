...
export default function TaskItem({ task }: { task: Task }) {
  ...
  if (isEditing) {
    return (
      <li className="flex items-center gap-2 p-3 rounded-xl border-2 border-violet-300 bg-violet-50">
        <input
          type="text"
          value={editTitle}
  ...
  return (
    <li
      className={`flex items-center gap-3 px-4 py-3 rounded-xl border transition-colors ${
        task.completed
          ? "bg-green-50 border-green-100"   // completed = soft green tint
          : "bg-gray-50 border-gray-100"     // incomplete = neutral
      }`}
    >
      {/* Toggle circle */}
      <button
      ...
      <button
        ...
        disabled={isPending}
        className="text-xs text-gray-300 hover:text-violet-500 font-semibold px-1.5 py-1 rounded-lg hover:bg-violet-50 transition-colors"
      ...
      <button
        ...
        disabled={isPending}
        className="text-xs text-gray-300 hover:text-red-400 font-semibold px-1.5 py-1 rounded-lg hover:bg-red-50 transition-colors"
